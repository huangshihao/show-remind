...
### 追加（同日）：netease_id 落库，回填全覆盖

上线后发现：网易歌单只能按 ID 逐个查头像，一次 resolve 只查前 30 名（按歌数排序），
且重复导入永远重查同样的 30 个——282 位艺人的歌单里 31 名以后永远没头像。修复
（migration 0005）：导入时把 `ar[].id` 存进 `artists.netease_id`；manage 后台回填
优先按 ID 精确查（网易资料确定无照片时清掉 ID 并给秀动一次机会，避免重试死循环），
`""`（秀动查过没有）但有 netease_id 的行重新纳入重试；单次回填量 15 → 30（外部
fetch 预算内，D1 写不计入）。效果：导一次歌单，之后每次页面加载后台自动补 30 个。
